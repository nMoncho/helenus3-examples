...
class HotelRepository()(using CqlSession):
    ...
    def findByPOI(poiName: String)(using ExecutionContext): Future[Seq[Hotel]] =
        queries.byPoi
    ...
    def findById(id: String)(using ExecutionContext): Future[Option[Hotel]] =
        queries.byId
            .executeAsync(id)
            .map(
    ...
    def findPOIsByHotel(id: String)(using ExecutionContext): Future[Seq[PointOfInterest]] =
    ...
    def availableRooms(id: String, date: LocalDate)(using ExecutionContext): Future[Set[HotelRepository.RoomNumber]] =
